    ...
    fun simulateGame() {
        when (whosTurn) {
            "white" -> {
                ...
                var piece = whitePawns.random()
                var next_move = piece.get_available_moves(board)
                piece.move(next_move.random(), board)
                whosTurn = "black"
            }
            "black" -> {
                println("Black To Move")
                var blackPawns = arrayOf(
                    blackPawn1,
                    blackPawn2,
                    blackPawn3,
                    blackPawn4,
